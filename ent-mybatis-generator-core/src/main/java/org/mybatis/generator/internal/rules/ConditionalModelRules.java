...
/**
 * This class encapsulates all the code generation rules for a table using the conditional
 * model. In this model we do not generate primary key or record with BLOBs classes if the
 * class would only hold one field.
 *
 * @author Jeff Butler
 *
 */
public class ConditionalModelRules extends BaseRules {
     ...
     * primary key coulmn (in which case we will not generate a primary key class), or if
     * there is only one BLOB column (in which case we will not generate a record with
     * BLOBs class).
    ...
    /**
     * We generate a record with BLOBs class if there is more than one BLOB column. Do not
     * generate a BLOBs class if any other super class would only contain one field
