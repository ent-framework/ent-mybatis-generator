...
public class IntrospectedColumn {
    ...
    public boolean isClobColumn() {
        String typeName = getJdbcTypeName();
        if ("VARCHAR".equals(typeName) && length> 2000) {
            return true;
        }

        return  "CLOB".equals(typeName) || "LONGNVARCHAR".equals(typeName) //$NON-NLS-1$ //$NON-NLS-2$
                 || "LONGVARCHAR".equals(typeName) //$NON-NLS-1$ //$NON-NLS-2$
                || "NCLOB".equals(typeName); //$NON-NLS-1$ //$NON-NLS-2$
    }
