...
public class MapstructPlugin extends AbstractServerPlugin {
	...
	private GeneratedJavaFile generateMapstruct(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
		...
		InnerInterface entityToVoConverter = createInnerMapper(mapstructInterface, "EntityToVoConverter", recordType,
				pojoRequestJavaType);
		...
		InnerInterface voToEntityConverter = createInnerMapper(mapstructInterface, "VoToEntityConverter",
				pojoRequestJavaType, recordType);
	...
	private Set<FullyQualifiedJavaType> addDefaultRelation(Set<FullyQualifiedJavaType> relations,
			FullyQualifiedJavaType recordType, InnerInterface innerInterface, boolean isRequest) {
		...
		relations.forEach(relationType -> {
			...
			if (!relationType.equals(recordType)) {
				if (isRequest) {
					importTypes.addAll(createDefaultConverterMethod(innerInterface, voJavaType, relationType));
				}
				else {
					importTypes.addAll(createDefaultConverterMethod(innerInterface, relationType, voJavaType));
				}
			}
