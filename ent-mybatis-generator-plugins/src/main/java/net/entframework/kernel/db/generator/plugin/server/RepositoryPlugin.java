...
public class RepositoryPlugin extends AbstractServerPlugin {
	...
	private GeneratedJavaFile generateRepositoryInterface(Interface clientInterface,
		...
		GeneratedJavaFile gjf = new GeneratedJavaFile(repositoryInterface,
	...
	private GeneratedJavaFile generateRepositoryImpl(Interface interfaze, IntrospectedTable introspectedTable,
		...
		// generate(repositoryInterfaceImplClass, new
		// RepDeleteByPrimaryKeyMethodGenerator(buildConfig));
		// generate(repositoryInterfaceImplClass, new
		// RepSelectByPrimaryKeyMethodGenerator(buildConfig));

		GeneratedJavaFile gjf = new GeneratedJavaFile(repositoryInterfaceImplClass,
				context.getJavaModelGeneratorConfiguration().getTargetProject(),
				context.getProperty(PropertyRegistry.CONTEXT_JAVA_FILE_ENCODING), context.getJavaFormatter());
		gjf.setOutputDirectory(getOutputDirectory());
		return gjf;
	...
	private AbstractMethodGenerator.BuildConfig getConfig(Interface clientInterface, AbstractJavaType interfaze,
		...
		return new AbstractMethodGenerator.BuildConfig().withContext(context)
